...
console.log('🔧 Final programs insertion (fixing JSON constraints)...');
...
connection.connect((err) => {
  ...
  connection.query('DELETE FROM Programs', (err) => {
    if (err) {
      ...
      // Insert all programs with proper JSON formatting
      console.log('📚 Inserting all 12 programs with proper JSON format...');
      ...
      // Insert programs without the requirements field (since it has JSON constraint)
      const insertQuery = `
        INSERT INTO Programs (
          title, description, category, durationValue, level, 
          curriculum, careerPaths, certificationName, 
          ...
          if (index === programs.length - 1) {
            setTimeout(() => {
              ...
              connection.query('SELECT COUNT(*) as count FROM Programs', (err, result) => {
                ...
                connection.query('SELECT id, title, category, durationValue FROM Programs ORDER BY id', (err, programs) => {
                  if (!err) {
                    console.log('\n📚 All programs in database:');
                    programs.forEach((prog, i) => {
                      console.log(`  ${prog.id}. ${prog.title} (${prog.category}, ${prog.durationValue} months)`);
                    });
                  }

                  connection.end();
                  console.log('\n🎉 All 12 programs successfully added to database!');
                  console.log('\n📋 Database is now ready for student registrations!');
                  console.log('✅ Students can now register and data will be saved to MySQL database');
                });
              });
            }, 1000);
          }
